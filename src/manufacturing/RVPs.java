...
class RVPs {
    ...
    protected Component.CompType uCompType() {
        ...
        if (typeDM.nextDouble() < PERCENT_A)
            type = Component.CompType.A;
        return (type);
    }

    // Processing Times procedure for M1, different for different component
    // types
    ...
    public double uProcTime(int machineId, Component.CompType type) {
        ...
        switch (machineId) {
        case Constants.M1:
            if (type == Component.CompType.A)
                processingTime = procM1A.nextDouble();
            else
                processingTime = procM1B.nextDouble();
            break;
        case Constants.M2:
